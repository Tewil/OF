...
namespace Broccoli.Builder
{
	...
	public class SproutMeshBuilder {
		...
		void PrepareData (BroccoTree tree, int sproutGroupId, SproutMesh sproutMesh, SproutMap.SproutMapArea sproutArea = null) {
			Clear ();
			...
			if (sproutArea != null && 
				sproutArea.enabled &&
				sproutMesh.overrideHeightWithTexture && 
				sproutArea.width > 0 && 
		...
		public Mesh MeshSprouts (BroccoTree tree, int sproutGroupId, SproutMesh sproutMesh, SproutMap.SproutMapArea sproutArea, int sproutAreaIndex, bool isTwoSided) {
			...
			PrepareData (tree, sproutGroupId, sproutMesh, sproutArea);
			List<BroccoTree.Branch> branches = tree.GetDescendantBranches ();
			for (int i = 0; i < branches.Count; i++) {
				for (int j = 0; j < branches[i].sprouts.Count; j++) {
					if (branches[i].sprouts[j].groupId == sproutGroupId && 
		...
		void MeshSprout (BroccoTree.Sprout sprout, BroccoTree.Branch branch, SproutMesh sproutMesh) {
			...
			case SproutMesh.Mode.Mesh:
				...
				break;
			case SproutMesh.Mode.GridPlane:
				AddGridPlaneMesh (sprout.inGirthPosition * globalScale, branch.position,
					sprout.sproutDirection, sprout.sproutNormal, 
			...
			ref Vector4[] planeUV2s) 
		{
			...
			if (planeDef.planes == 1) {
				...
				_normals.AddRange (GetBasePlaneNormals (sproutMesh.isTwoSided, true));
				_normals.AddRange (RotatePoints (GetBasePlaneNormals (sproutMesh.isTwoSided, true),
						Vector3.forward,
						Vector3.right));
				_uv2s.AddRange (SproutMeshMetaBuilder.GetInstance ().GetPlaneUV2s (
					planeDef.pivotPosW, planeDef.pivotPosH, 
		...
		Vector4[] GetBasePlaneTangents (bool isTwoSided) {
			...
			tangentsPerPlane[0] = Quaternion.Euler (0, 45, flat?90:35) * Vector3.forward;
			tangentsPerPlane[1] = Quaternion.Euler (0, 315, flat?90:35) * Vector3.forward;
			tangentsPerPlane[2] = Quaternion.Euler (0, 225, flat?90:35) * Vector3.forward;
			tangentsPerPlane[3] = Quaternion.Euler (0, 135, flat?90:35) * Vector3.forward;
			if (isTwoSided) {
				tangentsPerPlane[4] = Quaternion.Euler (0, 45, flat?90:145) * Vector3.forward;
				tangentsPerPlane[5] = Quaternion.Euler (0, 315, flat?90:145) * Vector3.forward;
				tangentsPerPlane[6] = Quaternion.Euler (0, 225, flat?90:145) * Vector3.forward;
				tangentsPerPlane[7] = Quaternion.Euler (0, 135, flat?90:145) * Vector3.forward;
			}
