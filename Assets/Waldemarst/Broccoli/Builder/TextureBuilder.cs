...
namespace Broccoli.Builder
{
    public class TextureBuilder {
        ...
        private void PositionCamera () {
            /*
            camera.orthographicSize = targetHeight / 2;
			...
			cameraGameObject.transform.position = targetCenter + angleVector.normalized * num;
			cameraGameObject.transform.LookAt (targetCenter);
            */
            camera.orthographicSize = (_targetMesh.bounds.max - _targetMesh.bounds.min).magnitude / 2;
			camera.aspect = 1f;
			// Positioning the camera
			float num = _targetMesh.bounds.extents.magnitude;
			camera.nearClipPlane = num * 0.1f;
			camera.farClipPlane = num * 2.2f;
			cameraGameObject.transform.position = _targetMesh.bounds.center + _cameraPlane.normal * num;
			cameraGameObject.transform.LookAt (_targetMesh.bounds.center, _cameraUp);
        }
        #endregion
    }
}