	...
	public class BranchMeshBuilder {
		...
		public Mesh MeshTree (BroccoTree tree) { // TODO: cut
			...
			for (int i = 0; i < tree.branches.Count; i++) {
				BranchSkin trunkBranchSkin = GetBranchSkin (tree.branches[i], 0);
		...
		public BranchSkin GetBranchSkin (BroccoTree.Branch firstBranch, int level) {
		...
		protected void PreprocessBranchSkin (BranchSkin branchSkin, BroccoTree.Branch firstBranch) {
			...
			for (int i = 0; i < branchSkin.ranges.Count; i++) {
				...
				if (branchMeshBuilder != null) {
					branchMeshBuilder.PreprocessBranchSkin (branchSkin, firstBranch);
				}
			}
			//Debug.Log ("This BranchSkin instance has " + branchSkin.ranges.Count + " ranges and " + branchSkin.builders.Count + " builders.");
		}
		/// <summary>
		/// Skins the branch.
		/// </summary>
		...
		void SkinBranch (BroccoTree.Branch branch, BranchSkin branchSkin, float consumedLength, bool isFollowup = false) {
			...
			for (int i = 0; i < branch.branches.Count; i++) {
				if (branch.branches[i] != branch.followUp) {
					BranchSkin childBranchSkin = GetBranchSkin (branch.branches[i], branchSkin.level + 1);
					PreprocessBranchSkin (childBranchSkin, branch.branches [i]);
		...
		protected virtual float SkinBranchMiddleBody (BroccoTree.Branch branch, float consumedLength, BranchSkin branchSkin, bool isFollowup = false) {
			...
			for (int i = 0; i < branchSkin.relevantPositions.Count; i++) {
				if (BranchSkin.PositionBelongsToBranch (branchSkin, branchSkin.relevantPositions[i], idToFirstBranch [branchSkin.id], branch.id, out inBranchPosition)) {
			...
			List<CurvePoint> curvePoints = branch.curve.GetPoints (branchAngleTolerance, relevantPositions);
		...
		protected void MeshBranchSkins (Dictionary<int, Mesh> branchMeshes) {
			...
			for (int i= 0; i < branchSkins.Count; i++) {
				// Clear structural data on the branch skin.
				branchSkins [i].Clear ();
				//branchSkins [i].vertexOffset = vertexOffset; // TODO: move to mesh merge
			...
			bool forceNewBaseVertices) 
		{
			...
			if (segmentIndex == branchSkin.segments.Count - 1) {
				...
				if (branchSkin.segments[segmentIndex] == 3) {
					...
					branchSkin.vertices.Add (branchSkin.centers [segmentIndex]);
