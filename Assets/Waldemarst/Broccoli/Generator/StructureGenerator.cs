...
namespace Broccoli.Generator
{
	...
	public class StructureGenerator {
			...
			if (!structureLevel.isSprout) {
				...
				if (structureLevel.enabled && structureLevel.probability >= probability) {
					...
					if (parentBranch != null) {
						parentBranch.Update ();
						parentBranch.RecalculateNormals (0f);
					}
					...
					if (preferredFrequency > 0) {
						...
						for (int i = 0; i < spawnedSprouts.Count; i++) {
							spawnedSprouts [i].CalculateVectors (parentBranch);
		...
		protected float GetLength (float positionAtBranch, StructureLevel level) {
			...
			*/
			if (level.id == 0) {
				return Random.Range (Random.Range (level.minLengthAtBase, level.maxLengthAtBase), Random.Range (level.minLengthAtTop, level.maxLengthAtTop));
			} else {
