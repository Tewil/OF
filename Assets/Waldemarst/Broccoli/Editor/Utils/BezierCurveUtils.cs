	...
	public class BezierCurveUtils {
		...
		public static void DrawCurve (BezierCurve curve, float resolution = 5f) {
			if (curve.points.Count > 1) {
				...
				if (curve.close) DrawCurve (curve[curve.points.Count - 1], curve[0], resolution);
			}
		}
		/// <summary>
		...
		public static void DrawCurve (BezierCurve curve, Color color, float width = 1f, float resolution = 5f) {
			...
			DrawCurve (curve.vectorPoints, color, width);
		}
		/// <summary>
		...
		/// <param name="color">Line color.</param>
		/// <param name="width">Line width.</param>
		public static void DrawCurve (List<Vector3> points, Color color, float width = 1f) {
			if (width == 1f) {
				Handles.color = color;
				...
				//Handles.DrawAAPolyLine (EditorDrawUtils.GetInstance().GetColoredTexture (color), width, points.ToArray());
			}
		}
		/// <summary>
