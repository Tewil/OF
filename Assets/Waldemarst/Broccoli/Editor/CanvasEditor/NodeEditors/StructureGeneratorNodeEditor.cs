...
namespace Broccoli.TreeNodeEditor
{
	...
	public class StructureGeneratorNodeEditor : BaseNodeEditor {
		...
		/// </summary>
		private int _editStructureId = -1;
		/// <summary>
		...
		/// </summary>
		/// <typeparam name="int">Id of curve.</typeparam>
		/// <returns></returns>
		...
		protected override void OnEnableSpecific () {
			...
			*/
			...
			propMinLength = propRootStructureLevel.FindPropertyRelative ("maxLengthAtBase");
			propMaxLength = propRootStructureLevel.FindPropertyRelative ("maxLengthAtTop");
			...
			TreeFactory.GetActiveInstance().forcePreviewModeColored = true;
			...
			for (int i = 0; i < branches.Count; i++) {
				for (int j = 0; j < branches[i].curve.nodes.Count; j++) {
					if (_nodeToCurve.ContainsKey(branches[i].curve.nodes[j].guid)) {
						...
						_selectedCurveIds.Add (branches[i].id);
					}
				}
		...
		public override void OnInspectorGUI () {
			...
			int selectedStructureId = curveEditor.selectedCurveId;
			...
			if (selectedStructureId >= 0 && curveEditor.hasSingleSelection && 
				structureGeneratorNode.structureGeneratorElement.idToStructure.ContainsKey (selectedStructureId)) {
				// ROOT LEVEL OPTIONS.
				EditorGUILayout.LabelField ("Selected Branch", EditorStyles.boldLabel);

				// Get the selected structure.
				StructureGenerator.Structure selectedStructure = structureGeneratorNode.structureGeneratorElement.idToStructure[selectedStructureId];
			...
			if (selectedLevel == null) {
				...
				IntRangePropertyField (propMinFrequency, propMaxFrequency, 0, 30, "Frecuency");
				ShowHelpBox (MSG_MAIN_MIN_MAX_FREQUENCY);

				// LENGTH
				FloatRangePropertyField (propMinLength, propMaxLength, 0.1f, 25f, "Length");
		...
		private bool DrawNodeCanvasControls () {
			...
			// Root Add Button
			if (GlobalSettings.experimentalRootsEnabled) {
			...
			if (GlobalSettings.experimentalSharedStructureLevelsEnabled) {
		...
		void DrawStructures (List<StructureGenerator.Structure> structures, int structureLevelId, Vector3 offset, float scale = 1) {
			...
			for (int i = 0; i < structures.Count; i++) {
				if (structures[i].generatorId == structureLevelId) {
					...
					_editStructureId = _editStructure.branch.id;
		...
		void UpdateVertexToSelection (List<int> selectedBranchIds) {
			/*
			 * x: branch id.
			 * y: generator id.
			...
			if (meshFilter != null) {
				...
				for (int i = 0; i < uv5s.Count; i++) {
					...
					if (selectedBranchIds.Contains ((int)uv5s[i].x)) {
						selectionTunedValue += 2;
					}
					uv5s[i] = new Vector4 (uv5s[i].x, uv5s[i].y, selectionTunedValue, uv5s[i].w);
				}
		...
		void OnSelectStructureLevel (Node node) {
			curveEditor.ClearSelection ();
			if (structureGeneratorNode.structureGeneratorElement.selectedLevel == null) {
				SetSelectedStructureLevel (structureGeneratorNode.structureGeneratorElement.rootStructureLevel.id);
			} else {
		...
		void OnNodeSelectionChanged (List<BezierNode> nodes, List<int> indexes, List<int> curveIds) {
		...
		void OnBeginMoveNodes (List<BezierNode> nodes, List<int> indexes, List<int> curveIds) {
			Undo.RecordObject (structureGeneratorNode.structureGeneratorElement, "Move Nodes");
		}
		/// <summary>
		...
		void OnMoveNodes (List<BezierNode> nodes, List<int> indexes, List<int> curveIds) {
		...
		/// <param name="nodes">Nodes to be moved.</param>
		/// <param name="indexes">Index of nodes to be moved.</param>
		void OnEndMoveNodes (List<BezierNode> nodes, List<int> indexes, List<int> curveIds) {
		...
		bool OnBeginMoveHandle (BezierNode node, int index, int curveId, int handle) {
		...
		/// <param name="curveId">Id of the curve the node.</param>
		/// <param name="handle">Number of the handle.</param>
		bool OnMoveHandle (BezierNode node, int index, int curveId, int handle) {
		...
		/// <param name="curveId">Id of the curve the node.</param>
		/// <param name="handle">Number of the handle.</param>
		bool OnEndMoveHandle (BezierNode node, int index, int curveId, int handle) {
			SetUndoControlCounter (false);
			return true;
		}
		/// <summary>
		...
		BezierCurveEditor.ControlType OnCheckNodeMoveControls (BezierNode node, int index, int curveId) {
