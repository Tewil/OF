...
namespace Broccoli.TreeNodeEditor
{
	...
	public class SproutMapperNodeEditor : BaseNodeEditor {
		...
		*/
		...
		/// </summary>
		SerializedProperty propMinColorShade;
		/// <summary>
		/// Maximum value for shading. The lower the darker.
		/// </summary>
		SerializedProperty propMaxColorShade;
		/// <summary>
		...
		*/
		...
		protected override void OnEnableSpecific () {
			...
			/*
			propCustomColors = GetSerializedProperty ("customColors");
			*/
			propColorVarianceMode = GetSerializedProperty ("colorVarianceMode");
			propMinColorShade = GetSerializedProperty ("minColorShade");
			propMaxColorShade = GetSerializedProperty ("maxColorShade");
		...
		public override void OnInspectorGUI() {
			...
			// Color variation.
			if (GlobalSettings.experimentalSproutMeshColorVariationEnabled) {
				EditorGUI.BeginChangeCheck ();
				EditorGUILayout.PropertyField (propColorVarianceMode, new GUIContent ("Color Variance"));
				if (propColorVarianceMode.enumValueIndex == (int)SproutMapperElement.ColorVarianceMode.Shades) {
					FloatRangePropertyField (propMinColorShade, propMaxColorShade, 0.65f, 1f, "Shade");
				}
		...
		private void DrawSproutMapElementTextureMode (SerializedProperty sproutMapProp, int index) {
			...
			if (MaterialManager.leavesShaderType != MaterialManager.LeavesShaderType.TreeCreatorOrSimilar) {
				EditorGUILayout.HelpBox ("SpeedTree official shaders do not support partial transparency (only Transparent Cutout).", MessageType.Warning);
			}
		...
		private void DrawSproutAreaElement (Rect rect, int index, bool isActive, bool isFocused) {
			...
			if (isActive) {
				...
				Texture2D texture = sproutArea.texture;
