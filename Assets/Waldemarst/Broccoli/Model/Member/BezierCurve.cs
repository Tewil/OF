...
namespace Broccoli.Model
{
	...
	public class BezierCurve {
		...
		/// </summary>
		private int cubicCurveSteps = 30;
		[SerializeField]
		/// <summary>
		...
		public void Process () {
            ...
            for (int i = 0; i < nodes.Count - 1; i++) {
                ...
                CubicBezierCurve cubicBezierCurve = new CubicBezierCurve (n, next, cubicCurveSteps, _autoProcess);
		...
		public void InsertNode (int index, BezierNode node) {
			nodes.Insert (index, node);
			node.curve = this;
			Process ();
		}
		public Broccoli.Model.BezierNode AddNodeAt (Vector3 position) {
			BezierNode newNode = new BezierNode (position);
			AddNode (newNode);
			return newNode;
		}
		public BezierNode AddNodeBehind( Vector3 position)
		{
			BezierNode newNode = AddNodeAt (position);
			InsertNode(0, newNode);
			return newNode;
		}
		public BezierNode InsertNodeAt (int index, Vector3 position)
		{
			BezierNode newNode = AddNodeAt (position);
		...
		public void SetResolution (int steps) {
			cubicCurveSteps = steps;
			for (int i = 0; i < bezierCurves.Count; i++) {
				bezierCurves[i].stepCount = cubicCurveSteps;
			}
		}
		/*
		...
		*/
		public Vector3 FindNearestNodeTo( Vector3 worldPos, float accuracy = 100f ) {
			float normalizedT;
			return FindNearestNodeTo( worldPos, out normalizedT, accuracy );
		}
		public Vector3 FindNearestNodeTo( Vector3 worldPos, out float normalizedT, float accuracy = 100f )	{
			if (nodes.Count <= 1) {
				normalizedT = 1f;
				return Vector3.zero;
			}
