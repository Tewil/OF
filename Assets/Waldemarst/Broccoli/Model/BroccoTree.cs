...
namespace Broccoli.Model {
	...
	public class BroccoTree {
		...
		public class Sprout {
			...
			/// <summary>
			/// Rotation of the sprout on its own longitudinal axis.
			/// </summary>
			public float rollAngle = 0f;
			/// <summary>
			/// Angle around the branch circumference.
			/// </summary>
			public float aroundBranchAngle = 0f;
			/// <summary>
			...
			/// </summary>
			public void CalculateVectors (Branch referenceBranch = null) {
				...
				if (referenceBranch != null) {
					...
					if (fromBranchCenter) {
						...
						Vector3 toGirthNormal = Quaternion.AngleAxis (aroundBranchAngle * Mathf.Rad2Deg + 90f, referenceBranch.GetDirectionAtPosition (position)) *
					...
					/*
					if (position == 1) {
						referenceBranchNormal = referenceBranch.GetNormalAtPosition (position - 0.001f);
						referenceBranchDirection = referenceBranch.GetDirectionAtPosition (position - 0.001f);
					} else {
						*/
					...
					result = Quaternion.AngleAxis(aroundBranchAngle * Mathf.Rad2Deg, GlobalSettings.againstGravityDirection) * result;
					Quaternion rotation;
					...
					resultN = Quaternion.AngleAxis(aroundBranchAngle * Mathf.Rad2Deg, GlobalSettings.againstGravityDirection) * resultN;
					if (referenceBranchNormal != Vector3.zero) {
					...
					if (horizontalAlign > 0) {
						Vector3 horizontalDirection = Vector3.ProjectOnPlane (sproutDirection, Vector3.up);
					...
					// Gravity Align
					if (gravityAlign != 0) {
						Vector3 newSsproutDirection = Vector3.Lerp (
							sproutDirection, 
						...
						Quaternion gravityRotation = Quaternion.FromToRotation (sproutDirection, newSsproutDirection);
			...
			public Sprout Clone () {
				...
				clone.aroundBranchAngle = aroundBranchAngle;
		...
		public class Branch {
			...
			public void UpdatePosition (Branch referenceBranch = null) {
				if (_parent != null) {
					if (_parent.followUp == this) {
						//positionFromBranch = _parent.curve.GetPointAt(position, true).position;
						//positionFromBranch = _parent.curve.GetPointAt(position).position - curve.GetPointAt(0f).position;
					...
					positionFromBranch = referenceBranch.curve.GetPointAt(position).position;
			...
			public void UpdateResolution (int resolutionSteps, bool recursive = false) {
				curve.SetResolution (resolutionSteps);
			...
			/// </summary>
			/// <param name="segmentAngle">Segment angle reference to calculate the normals.</param>
			public void RecalculateNormals (float segmentAngle) {
				RecalculateNormals (Vector3.zero, Vector3.zero, segmentAngle);
			}
			/// <summary>
			/// Recalculates the normals.
			/// </summary>
			...
			/// <param name="segmentAngle">Segment angle reference to calculate the normals.</param>
			public void RecalculateNormals (Vector3 prevDirection, Vector3 prevNormal, float segmentAngle) {
				if (isTrunk) {
				...
				*/
				...
				for (int i = 0; i < _branches.Count; i++) {
					if (_branches[i] == followUp) {
						_branches[i].RecalculateNormals (prevDirection, prevNormal, segmentAngle);
					} else {
						_branches[i].RecalculateNormals (Vector3.zero, Vector3.zero, segmentAngle);
					}
				}
			}
			/// <summary>
		...
		/// Recalculates the normals.
		/// </summary>
		/// <param name="segmentAngle">Segment angle reference to recalculate normals.</param>
		public void RecalculateNormals (float segmentAngle) {
			for (int i = 0; i < _branches.Count; i++) {
				_branches[i].RecalculateNormals (segmentAngle);
			}
		}
