...
namespace Broccoli.Factory
{
	...
	public class TreeFactory : MonoBehaviour {
		...
		/// <summary>
		/// Stablishes the method for building the tree, whether
		/// it is for preview purposes or to create a prefab (optimized).
		/// </summary>
		...
		public GameObject Spawn (int seed) {
			...
			if (_localPipeline != null) {
				...
				*/
				...
				InitPreviewTree ();
				ProcessPipeline (_localPipeline, _previewTree, null, true, pass, false);
			...
			bool forceNewTree = false)
		{
			if (onBeforeProcessPipeline != null) {
				onBeforeProcessPipeline (pipeline, tree, referenceElement, useCache, pass, forceNewTree);
			}
			bool result = false;
			...
			if (pipeline != null) {
				if (ValidatePipeline (pipeline)) {
					...
					TreeFactoryProcessControl processControl = new TreeFactoryProcessControl (referenceElement, pass);
					do {
			...
			if (isPreview && !treeFactoryPreferences.prefabStrictLowPoly && pass == 1) {
			...
			if (onProcessPipeline != null) {
				onProcessPipeline (pipeline, tree, referenceElement, useCache, pass, forceNewTree);
			}
		...
		public bool ProcessPipelinePreview (PipelineElement referenceElement = null, bool useCache = false, bool forceRebuild = false) {
			...
			if (onBeforeProcessPipelinePreview != null) {
				onBeforeProcessPipelinePreview (_localPipeline, _previewTree, referenceElement, useCache, 1, forceRebuild);
			}
			bool result = ProcessPipeline (_localPipeline, _previewTree, referenceElement, useCache, 1, forceRebuild);
			if (onProcessPipelinePreview != null) {
				onProcessPipelinePreview (_localPipeline, _previewTree, referenceElement, useCache, 1, forceRebuild);
			}
			return result;
		}
		/// <summary>
		...
		public bool ProcessPipelinePreviewDownstream (PipelineElement referenceElement, bool useCache = false) {
			isPreview = true;
			if (onBeforeProcessPipelinePreview != null) {
				onBeforeProcessPipelinePreview (_localPipeline, _previewTree, referenceElement, useCache);
			}
			bool result = ProcessPipeline(_localPipeline, _previewTree, referenceElement, useCache);
			if (onProcessPipelinePreview != null) {
				onProcessPipelinePreview (_localPipeline, _previewTree, referenceElement, useCache);
			}
			return result;
		}
		/// <summary>
			...
			bool useCache = false) 
		{
			...
			if (upstreamElement != null) {
				...
				bool result = ProcessPipeline(_localPipeline, _previewTree, upstreamElement, useCache);
				if (onProcessPipelinePreview != null) {
					onProcessPipelinePreview (_localPipeline, _previewTree, referenceElement, useCache);
				}
				return result;
		...
		/// <summary>
		/// Determines whether this instance is in preview mode processing.
		/// </summary>
		/// <returns><c>true</c> if this instance is in preview mode processing; otherwise, <c>false</c>.</returns>
		...
		public int GetAtlasSize (TextureSize atlasTextureSize) {
			if (atlasTextureSize == TextureSize._128px) {
				return 128;
			} else if (atlasTextureSize == TextureSize._256px) {
				return 256;
		...
		private void SetMaterials (MeshRenderer meshRenderer, int neededMaterialsCount) {
			...
			for (int i = 0; i < neededMaterialsCount; i++) {
				...
				if (treeFactoryPreferences.previewMode == PreviewMode.Colored || forcePreviewModeColored) {
					// Colored materials.
