		...
		*/
		...
		public void BeginWithCreatePrefab (GameObject previewTarget) {
			...
			if (!AssetDatabase.IsValidFolder (GlobalSettings.prefabSavePath)) {
				...
				int i = 0;
				do {
					prefabName = GlobalSettings.prefabSavePrefix + i;
		...
		public bool EndWithCommit (bool generateBillboard) {
			bool result = SavePrefab (generateBillboard);
			#if UNITY_EDITOR
		...
		bool SavePrefab (bool generateBillboard) {
			#if UNITY_EDITOR
			if (prefabIsValid) {
				...
				if (assetMeshes.Count == 1 && !generateBillboard) {
					...
					while (enumerator.MoveNext ()) {
						...
						if (materialParameters.ContainsKey (materialPair.Key)) {
							...
							case MaterialParams.ShaderType.Native:
								SetNativeMaterial (materialPair.Value [0]);
								break;
					...
					if (generateBillboard) {
						billboardBuilder.billboardTexturePath = GetPrefabTextureFolder () + "/billboard.png";
						billboardBuilder.billboardNormalTexturePath = GetPrefabTextureFolder () + "/billboard_normal.png";
		...
		void SetNativeMaterial (Material nativeMaterial) {
			#if UNITY_EDITOR
